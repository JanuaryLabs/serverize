...
const build: (config: { projectFile: string; version: string }) => Stage = (
  config,
) => ({
...
export const dotnet = (config: {
  ...
  dockerfile({
    stages: {
      build: stage(build, {
        projectFile: `${config.projectName}.csproj`,
    ...
    start: {
      ...
      healthCheck: {
        test: `curl -f http://localhost:8080/${config.healthCheck} || exit 1`,
      },
    },
  });
dotnet.dockerignore = [
